...
export type ChicoState = 'rich' | 'thriving' | 'okay' | 'stressed' | 'shocked' | 'eating' | 'hungry';

export const CHICO_PALETTE = {
...
type ExpressionProps = {
  eyeShape: 'round' | 'happy' | 'star' | 'shock' | 'worried';
  eyeH?: number;
  browL: string;
  browR: string;
  mouth: 'bigSmile' | 'smile' | 'flat' | 'frown' | 'oh' | 'open' | 'chomp';
  arms: 'up' | 'side' | 'cover' | 'reach';
  blush: boolean;
  tuft?: string;
};

export const CHICO_EXPRESSIONS: Record<string, ExpressionProps> = {
  rich: {
    eyeShape: 'star',
...
export function chicoStateFromSavings(pct: number): ChicoState {
...
export const CHICO_LINES: Record<string, string[]> = {
  rich: [
...
export function chicoLine(state: ChicoState, seed = 0): string {
...
interface ChicoProps {
  state?: ChicoState;
  size?: number | string;
  palette?: typeof CHICO_PALETTE;
  animate?: boolean;
}

export function Chico({ state = 'okay', size = 200, palette = CHICO_PALETTE, animate = true }: ChicoProps) {
  const p = palette;
