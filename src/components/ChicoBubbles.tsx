...
import { Chico, ChicoState } from './Chico';
import { Palette, CT_SEMANTIC } from '@/lib/tokens';
import { Allocation } from '@/lib/types';

const BUBBLE_LIBRARY: Allocation[] = [
...
interface ChicoDropContextType {
  setChicoRef: (r: React.RefObject<HTMLDivElement | null>) => void;
  chompPulse: number;
  eating: boolean;
  near: boolean;
  dragColor?: string;
...
export const ChicoDropContext = createContext<ChicoDropContextType | null>(null);
...
interface BubbleStageProps {
  ...
  chicoState: ChicoState;
  onChicoStateOverride?: (s: 'eating' | 'hungry' | null) => void;
}

export function BubbleStage({ palette, children, allocations, onAdd, onRemove, onEditAllocation, chicoState, onChicoStateOverride }: BubbleStageProps) {
  const p = palette;
  const stageRef = useRef<HTMLDivElement>(null);
  const chicoRefHolder = useRef<React.RefObject<HTMLDivElement | null> | null>(null);
  const setChicoRef = (r: React.RefObject<HTMLDivElement | null>) => { chicoRefHolder.current = r; };

  const usedIds = new Set(allocations.map(a => a.id));
  const visibleBubbles = BUBBLE_LIBRARY.filter(b => !usedIds.has(b.id));

  const [drag, setDrag] = useState<{ id: string | number; bubble: Allocation; x: number; y: number } | null>(null);
  const [chompPulse, setChompPulse] = useState(0);
  const [flash, setFlash] = useState<Allocation | null>(null);
  ...
  const startDrag = (bubble: Allocation, e: React.PointerEvent) => {
    e.preventDefault();
  ...
  const saveEdit = (updated: Allocation) => {
    onAdd(updated);
    setEditing(null);
    setChompPulse(p => p + 1);
    setEating(true);
...
function BubbleChip({ bubble, index, onPointerDown, hidden }: { bubble: Allocation, index: number, onPointerDown: (e: any) => void, hidden: boolean }) {
  return (
...
function BubbleChipBody({ bubble, active = false }: { bubble: Allocation, active?: boolean }) {
  return (
    <div style={{
...
function FlashChip({ flash, chicoRect, stageRect }: { flash: Allocation, chicoRect: DOMRect, stageRect: DOMRect }) {
...
export function BubbleEditSheet({ palette, bubble, onSave, onCancel }: { palette: Palette, bubble: Allocation, onSave: (a: Allocation) => void, onCancel: () => void }) {
  const p = palette;
  const [draft, setDraft] = useState(bubble);

  const update = (k: keyof Allocation, v: any) => setDraft(prev => ({ ...prev, [k]: v }));
  ...
  return (
    <>
      ...
      <div style={{
        ...
        fontFamily: 'var(--font-sans)', color: p.ink,
      }}>
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <input
              ...
              style={{
                ...
                fontFamily: 'var(--font-serif)', fontSize: 24, color: p.ink, padding: 0,
                outline: 'none',
              }}
            />
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginTop: 2 }}>
            <span style={{ fontSize: 24, color: p.inkSoft, fontFamily: 'var(--font-serif)' }}>₱</span>
            <input
              ...
              style={{
                ...
                fontFamily: 'var(--font-serif)', fontSize: 32, color: draft.color, padding: 0,
...
export function AllocationStack({ palette, allocations, onRemove, onEdit }: { palette: Palette, allocations: Allocation[], onRemove: (id: string | number) => void, onEdit?: (a: Allocation) => void }) {
  const p = palette;
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {allocations.map(a => (
          <span key={a.id} style={{
            ...
            fontSize: 11, color: p.ink, fontFamily: 'var(--font-sans)',
          }}>
...
export function DroppableChico({ state, size, palette }: { state: ChicoState, size: number, palette?: Palette }) {
  ...
  return (
    <div ref={wrapRef} style={{
      ...
      `}</style>
      <Chico state={effectiveState as ChicoState} size={size} />
      {crumbs.map(c => (
        <span key={c.id} style={{
          ...
          pointerEvents: 'none',
        } as React.CSSProperties & { '--cx': string }} />
      ))}
    </div>
  );
}
